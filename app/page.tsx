    ...
    <div className="min-h-screen bg-[#e2e8f0] text-slate-900 flex flex-col justify-between selection:bg-amber-500 selection:text-white">
      ...
      <header className="w-full px-6 sm:px-12 py-5 flex items-center justify-between border-b border-slate-300 backdrop-blur-md sticky top-0 z-50 bg-[#f8fafc]/95 shadow-sm">
        ...
        <div className="flex items-center gap-4">
          <Link href="/login" className="text-sm font-bold text-slate-700 hover:text-slate-900 transition">
            Entrar
          </Link>
      ...
      <main className="w-full px-6 sm:px-12 py-16 text-center space-y-12">
        ...
        <div className="inline-flex items-center gap-2 bg-amber-100 border border-amber-300 text-amber-900 text-xs font-bold px-4 py-1.5 rounded-full shadow-inner">
        ...
        <p className="text-lg sm:text-xl text-slate-700 max-w-3xl mx-auto leading-relaxed font-semibold">
        ...
        {/* COMO FUNCIONA */}
        <div className="pt-8 pb-6 border-y border-slate-300 my-6 max-w-6xl mx-auto">
          <div className="text-center space-y-2 mb-10">
            <span className="text-amber-800 text-xs font-extrabold uppercase tracking-wider bg-amber-100 px-3 py-1 rounded-full border border-amber-300">Passo a Passo</span>
            <h2 className="text-2xl sm:text-4xl font-black text-slate-900">Como cuidar de quem você ama em 3 passos</h2>
            <p className="text-sm text-slate-700 font-semibold">Entenda como é simples proteger seu pet e garantir que ele volte para casa.</p>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 text-left">
            
            <div className="bg-[#f8fafc] border border-slate-300 p-6 rounded-3xl space-y-3 relative overflow-hidden group hover:border-slate-400 shadow-md transition">
              <div className="text-3xl font-black text-slate-400/40 absolute top-4 right-4">01</div>
              <div className="text-2xl">📝</div>
              <h3 className="font-bold text-slate-900 text-lg">Cadastre com carinho</h3>
              <p className="text-sm text-slate-700 leading-relaxed font-semibold">Insira o nome, a foto e os dados de contato. O primeiro pet da família tem ativação 100% gratuita. Para os demais, cobramos apenas uma taxa única de R$ 10,00.</p>
            </div>

            <div className="bg-[#f8fafc] border border-slate-300 p-6 rounded-3xl space-y-3 relative overflow-hidden group hover:border-slate-400 shadow-md transition">
              <div className="text-3xl font-black text-slate-400/40 absolute top-4 right-4">02</div>
              <div className="text-2xl">🖨️</div>
              <h3 className="font-bold text-slate-900 text-lg">Monte a plaqueta</h3>
              <p className="text-sm text-slate-700 leading-relaxed font-semibold">Baixe o molde em PDF na hora, imprima em casa e coloque na coleira. Uma ponte direta entre o seu pet e o seu abraço.</p>
            </div>

            <div className="bg-[#f8fafc] border border-slate-300 p-6 rounded-3xl space-y-3 relative overflow-hidden group hover:border-slate-400 shadow-md transition">
              <div className="text-3xl font-black text-slate-400/40 absolute top-4 right-4">03</div>
              <div className="text-2xl">📍</div>
              <h3 className="font-bold text-slate-900 text-lg">Resgate imediato</h3>
              <p className="text-sm text-slate-700 leading-relaxed font-semibold">Se ele se perder, qualquer pessoa de bom coração que o encontrar poderá ler o QR Code e avisar onde ele está num piscar de olhos.</p>
            </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 pt-4 max-w-4xl mx-auto text-center">
          <div className="bg-[#f8fafc] border border-emerald-300 p-4 rounded-2xl shadow-md">
            <p className="text-2xl sm:text-3xl font-extrabold text-emerald-700">R$ 0,00</p>
            <p className="text-xs text-slate-700 font-bold">Ativação gratuita para o 1º pet</p>
          </div>
          <div className="bg-[#f8fafc] border border-slate-300 p-4 rounded-2xl shadow-md">
            <p className="text-2xl sm:text-3xl font-extrabold text-slate-900">R$ 10,00</p>
            <p className="text-xs text-slate-700 font-bold">Taxa única para pets adicionais</p>
          </div>
          <div className="col-span-2 sm:col-span-1 bg-[#f8fafc] border border-slate-300 p-4 rounded-2xl shadow-md">
            <p className="text-2xl sm:text-3xl font-extrabold text-amber-600">Paz Mental</p>
            <p className="text-xs text-slate-700 font-bold">Contato direto com quem achou</p>
          </div>
        </div>
        ...
        <div className="pt-10 pb-8 max-w-5xl mx-auto w-full">
          <div className="bg-amber-600 border border-amber-500 p-8 sm:p-12 rounded-3xl space-y-6 shadow-2xl text-white">
            ...
            <p className="text-amber-50 text-sm sm:text-base max-w-xl mx-auto font-semibold">
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center gap-4 pt-2">
              ...
              <Link 
                href="/login" 
                className="w-full sm:w-auto bg-slate-900 hover:bg-slate-800 text-white font-semibold text-base px-8 py-4 rounded-2xl transition border border-amber-500/40 shadow-lg"
              >
                Acessar Minha Conta
              </Link>
            </div>
      ...
      <footer className="w-full px-6 sm:px-12 py-8 border-t border-slate-300 text-center text-xs text-slate-600 flex flex-col sm:flex-row items-center justify-between gap-4 font-bold">
        ...
        <div className="flex gap-4">
          ...
          <Link href="/register" className="hover:text-slate-900 transition">Criar Conta</Link>
        </div>
      </footer>
